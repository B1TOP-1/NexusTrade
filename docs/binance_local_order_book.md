...
| 速度 | 间隔 | 说明 |
|---|---|---|
| `@depth@0ms` | **尽可能实时** | ✅ 实测可用（VPS+本地验证），~29 事件/秒（100ms 的 3 倍） |
| `@depth@100ms` | **100ms** | ✅ 官方文档标注最快 |
| `@depth`（无后缀） | **250ms** | 默认 |
| `@depth@250ms` | 250ms | 等同于无后缀 |
| `@depth@500ms` | 500ms | 低频 |

> `@0ms` 曾被认为废弃，但实测（2026-08-11）**可用**且推送更密集。
> 它是"尽可能实时推送"模式（非固定 0ms 间隔）。通过环境变量启用：
> `BINANCE_FUTURES_DEPTH_UPDATE_SPEED=0ms`
